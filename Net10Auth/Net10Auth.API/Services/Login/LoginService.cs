...
public class LoginService(UserManager<ApplicationUser> userManager, SignInManager<ApplicationUser> signInManager, IConfiguration configuration, IOptions<JwtConfiguration> jwtConfiguration) : ILoginService
{
    public async Task<CtrlResponse<LoginResponse>> LoginAsync(LoginInputModel model, HttpRequest httpRequest)
    {
        ...
        if (user.Email.IsNullOrWhiteSpace()) return new CtrlResponse<LoginResponse>("Email is null");
         ...
         var signInResult = await signInManager.CheckPasswordSignInAsync(user, model.Password ?? throw new InvalidOperationException(),  true);
        if (signInResult is { Succeeded: false, IsLockedOut: true } ) return new CtrlResponse<LoginResponse>("user is locked out");
        ...
        return new CtrlResponse<LoginResponse>("");
    }
}